...
export default function Page() {
  ...
  async function handleGenerate() {
    if (!docType.trim() || !details.trim()) return;
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          data.error || data.generatedDoc || "Something went wrong",
        );
      }

      setGeneratedDoc(data.generatedDoc || "No result generated.");
    } catch (error: any) {
      console.error(error);

      setGeneratedDoc(error.message || "Something went wrong.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-4xl font-bold mb-6">Legal Document Generator</h1>
        ...
        {generatedDoc && (
          <div className="mt-8 p-6 rounded-lg bg-zinc-900 border border-zinc-700">
            ...
            <div className="whitespace-pre-wrap">{generatedDoc}</div>
          </div>
        )}
      </div>
    </div>
