...
Each feature now has its own Genkit-style flow in `src/ai/flows/feature-flows.ts` (for example `chatbotFlow`, `groundedSearchFlow`, `healthPlannerFlow`, and `presentationGuideFlow`). The Next.js API routes call those feature-specific flows directly, so prompts, schemas, and output parsing are implemented in the AI layer instead of being duplicated in route handlers.
